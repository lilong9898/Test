...
import com.squareup.leakcanary.internal.*
 ...
 * (1) 继承自{@link android.app.IntentService}
 ...
 * (2) 其中的{@link WatchExecutor#execute(Retryable)}方法会将输入的retryable在主线程空闲的时候,发送到其创建的一个{@link android.os.HandlerThread}
 *
 ...
 * (2) {@link AndroidHeapDumper#dumpHeap()}中用{@link android.os.Debug#dumpHprofData(String)}方法将堆内存快照写入文件
 *
 * -----------------------高层流程---------------------------------
 * {@link LeakCanary#install(android.app.Application)},其中
 *   (1) 通过{@link LeakCanary#refWatcher(android.content.Context)}方法创建{@link AndroidRefWatcherBuilder}
 ...
 *       (4.2) 通过{@link ActivityRefWatcher#install(android.content.Context, RefWatcher)}方法向所有activity注册泄漏监听
 *             具体监听方式是,通过{@link android.app.Application#registerActivityLifecycleCallbacks(android.app.Application.ActivityLifecycleCallbacks)}
 *             监听所有activity的onDestroy方法,一旦触发,就调(4.1)生成的watcher的{@link RefWatcher#watch(Object)}方法开始跟踪这个activity的引用
 *       (4.3) 通过{@link FragmentRefWatcher.Helper#install(android.content.Context, RefWatcher)}方法向所有fragment注册泄漏监听
 *             具体监听方式是,通过{@link android.app.Application#registerActivityLifecycleCallbacks(android.app.Application.ActivityLifecycleCallbacks)}
 *             监听所有activity的onActivityCreated方法,一旦触发,就调{@link FragmentRefWatcher#watchFragments(android.app.Activity)}方法,
 *             这个方法内部会获取到activity的FragmentManager, 并通过{@link android.support.v4.app.FragmentManager#registerFragmentLifecycleCallbacks(android.support.v4.app.FragmentManager.FragmentLifecycleCallbacks, boolean)}
 ...
 *            ->{@link HeapAnalyzerService#runAnalysis(android.content.Context, HeapDump, Class)}
 ...
 *          (2.1) 创建一个指向{@link DisplayLeakActivity}的{@link android.app.PendingIntent}
 *          (2.2) 用这个{@link android.app.PendingIntent}创建一个通知, 点击通知会打开{@link DisplayLeakActivity}
 *
