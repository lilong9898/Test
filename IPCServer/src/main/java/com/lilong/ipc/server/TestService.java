...
public class TestService extends Service {
    ...
    class StubImpl extends ITest.Stub{
         ...
         *     binder机制基于mmap调用，将接收方进程的用户空间一块内存(叫用户空间binder内存)和内核空间的一块内存(叫内核空间binder内存)都映射到同一块物理内存上
         *     发送方进程的用户空间中的数据通过binder驱动写入到公用的内核空间binder内存上，按照mmap原理，接收方进程的用户空间binder内存就能看到这个数据
